...
<section class="show-posts">
   ...
   <div class="box-container">
      ...
      <?php
         ...
         if($select_posts->rowCount() > 0){
            while($fetch_posts = $select_posts->fetch(PDO::FETCH_ASSOC)){
               ...
               $user_id = $fetch_posts['admin_id'];
               $postedByUserQuery = $conn->prepare("SELECT * FROM `admin` WHERE id = ?");
               $postedByUserQuery->execute([$user_id]);
               $postedByUser = $postedByUserQuery->fetch(PDO::FETCH_ASSOC);
      ...
      <form method="post" class="box">
         ...
         <div class="icons post-user">
            <div class="likes">
               <i class="fas fa-user"></i>
               Posted by <?= $postedByUser['id'] == $admin_id ? "You" : $postedByUser['name'] ?>
