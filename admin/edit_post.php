...
if(isset($_POST['save'])){
   ...
   $update_post = $conn->prepare("UPDATE `posts` SET title = ?, content = ?, topic_id = ?, status = ? WHERE id = ?");
   $update_post->execute([$title, $content, $topic_id, $status, $post_id]);
...
<section class="post-editor">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <select name="topic_id" class="box" required>
         <?php
            $topicsQuery = $conn->prepare("SELECT * FROM `topics`");
            $topicsQuery->execute();
            while($topic = $topicsQuery->fetch(PDO::FETCH_ASSOC)) {
            ?>                     
                     
         <option value="<?= $topic['id'] ?>" selected="<?= $topic['id'] ==  $fetch_posts['topic_id'] ?>"><?= $topic['name'] ?></option>
         <?php } ?>
